...
ShaderTestsModule::ShaderTestsModule(
    ...
    auto shader_combined_two_parts_test = addTest("shader_combined_two_parts", [&](test::Test& test) { shaderCombinedTwoPartsTest(test); });
}
...
void ShaderTestsModule::shaderCombinedTwoPartsTest(test::Test& test) {
    window.setSize(WINDOW_SIZE);
    window.setTitle("shader combined two parts");
    View view;
    view.setPosition(window.getCenter());
    window.setView(view);
